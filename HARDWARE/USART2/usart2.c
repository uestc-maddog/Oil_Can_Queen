...
//串口接收缓存区 	
volatile u8 USART2_RX_BUF[USART2_MAX_RECV_LEN]; 				//接收缓冲,最大USART2_MAX_RECV_LEN个字节.

extern volatile u8 Join_Scan;           // Join AT指令标志
...
void USART2_IRQHandler(void)
{
	...
	{	 
		...
		{
			USART2_RX_BUF[USART2_RX_STA++]=res;		    // 记录接收到的值
			if((USART2_RX_BUF[(USART2_RX_STA&0X7FFF)-2] == 'O') && (res == 'K'))
			{
